                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="h-16 w-16 bg-gray-100 rounded-lg flex items-center justify-center">
                                    @if($product->image)
                                        <img src="{{ asset('assets/' . $product->image) }}" alt="{{ $product->name }}" class="h-12 w-12 object-contain">
                                    @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form action="{{ route('admin.products.toggle-featured', $product) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium transition duration-300
                                        {{ $product->featured ? 'bg-green-100 text-green-800 hover:bg-green-200' : 'bg-gray-100 text-gray-800 hover:bg-gray-200' }}">
                                        @if($product->featured)
